...
struct Functionals {
    ...
    Functionals(const SymbolMethod &symbolMethod) {
        ...
        auto patch_it = patchesTable.find(systemVersion);
        if (patch_it != patchesTable.end()) {
            for (const auto &[patchTo, signature] : patch_it->second) {
    ...
    GetInstance(const SymbolMethod &symbolMethod = {dlopen, dlsym, dlclose}) {
    ...
    SurfaceControl CreateSurface(const char *name, int32_t width,
        ...
        auto result =
            Functionals::GetInstance().SurfaceComposerClient__CreateSurface(
                data, windowName, width, height, 1, flag,
        ...
        if (12 <= Functionals::GetInstance().systemVersion &&
            Functionals::GetInstance().SurfaceComposerClient__Transaction__SetTrustedOverlay) {
...
public:
    ...
    static void *GetSurfaceNative(ANativeWindow *nativeWindow) {
        if (!nativeWindow)
            return nullptr;
        auto it = m_cachedSurfaceControl.find(nativeWindow);
        if (it == m_cachedSurfaceControl.end())
            return nullptr;
        if (nullptr == detail::Functionals::GetInstance().SurfaceControl__GetSurface)
            return nullptr;
        auto result = detail::Functionals::GetInstance().SurfaceControl__GetSurface(
            it->second.data);
