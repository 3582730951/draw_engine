...
PATTERNS = {
    ...
    "SurfaceComposerClient_ctor": [r".*SurfaceComposerClient.*C[12]E.*"],
    "SurfaceComposerClient_dtor": [r".*SurfaceComposerClient.*D[012]E.*"],
    "SurfaceComposerClient_getDefault": [r".*SurfaceComposerClient.*getDefault.*"],
    ...
    "SurfaceComposerClient_Transaction_apply": [
        r".*SurfaceComposerClient.*Transaction.*apply.*"
    ],
    ...
    "ASurfaceTransaction_create": [r"^ASurfaceTransaction_create$"],
    "ASurfaceTransaction_release": [r"^ASurfaceTransaction_release$"],
    "ASurfaceTransaction_apply": [r"^ASurfaceTransaction_apply$"],
...
def parse_sdk_from_path(path: str) -> int:
    parts = path.replace("\\", "/").split("/")
    for part in reversed(parts):
        m = re.search(r"(?:android-)?(\d{2})", part.lower())
        if not m:
            continue
        major = int(m.group(1))
        if major in SDK_MAP:
...
def collect_versions(roots):
    ...
    for root in roots:
        for dirpath, _, filenames in os.walk(root):
            if (
                "libgui.so" in filenames
                or "libandroid.so" in filenames
                or "libutils.so" in filenames
            ):
                sdk = parse_sdk_from_path(dirpath)
...
def generate_header(out_path: str, versions: dict, nm_path: str):
    version_entries = []
    for path, sdk in sorted(versions.items(), key=lambda kv: kv[1]):
        ...
        if not os.path.exists(libgui) and not os.path.exists(libandroid) and not os.path.exists(
            libutils
        ):
            continue
    ...
    all_results = []
    for _, sdk, libgui, libandroid, libutils in version_entries:
        symbols_gui = run_nm(nm_path, libgui) if os.path.exists(libgui) else set()
        symbols_android = (
            run_nm(nm_path, libandroid) if os.path.exists(libandroid) else set()
        )
        symbols_utils = (
            run_nm(nm_path, libutils) if os.path.exists(libutils) else set()
        )
        results = {}
        for logical, patterns in PATTERNS.items():
            ...
            if hint == "libutils":
                ...
                src = symbols_android if "ANativeWindow" in logical else (
                    symbols_gui | symbols_android
                )
            else:
                src = symbols_gui
...
def main():
    ...
    generate_header(args.out, versions, args.nm)
